...
int angle1; // variable to store analog value
float voltage;
...
void loop() {
  ...
  filtered = conv_rt(angle1);  //filtered = readADC_Avg(angle1, AN_Pot1_Buffer);
  voltage = angle1*3.3/4095;
  //float voltfiltered = filtered*3.3/4095;
  float anglemap = mapfloat((float) angle1, 0, 4095, 0, 333.3);
  float anglefilt = mapfloat((float) filtered, 0, 4095, 0, 333.3);
  ...
  Serial.print(anglemap); Serial.print(","); 
  //Serial.print(", voltage filtered: "); Serial.print(voltfiltered);
  //Serial.print(", angle: "); 
  //Serial.print(anglemap); Serial.print(","); 
